...
(let ((time-start (now-us)) (time-stop 0) (n (expt 10 100000)))
  ;; (format t "square-of-sum-reduce() result: ~A~%" (difference-of-squares:square-of-sum n)) ; this adds ~4 seconds to print the huge number
  ;; (sleep 1)
  (difference-of-squares:square-of-sum n)
  (setq time-stop (now-us))
  (format t " square-of-sum-reduce()   time: ~A microseconds for 10^100,000~%" (- time-stop time-start))
  )

(let ((time-start (now-us)) (time-stop 0) (n (expt 10 100000)))
  ;; (format t "sum-of-squares-reduce() result: ~A~%" (difference-of-squares:sum-of-squares n)) ; this adds ~4 seconds to print the huge number
  ;; (sleep 1)
  (difference-of-squares:sum-of-squares n)
  (setq time-stop (now-us))
  (format t "sum-of-squares-reduce()   time: ~A microseconds for 10^100,000~%" (- time-stop time-start))
  )
