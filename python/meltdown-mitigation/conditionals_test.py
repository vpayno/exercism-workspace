...
class MeltdownMitigationTest(unittest.TestCase):
    ...
    def test_get_efficiency_rate(self):
        ...
        for variant, data in enumerate(test_data, start=1):
            ...
            with self.subTest(
                ...
                # pylint: disable=assignment-from-no-return
                actual_result = get_efficiency_rate(voltage, current, theoretical_max_power)
                failure_message = (
                    f"Expected {expected} but returned {actual_result} "
                    f"with voltage={voltage}, current={current}, max_pow={theoretical_max_power}"
                )
    ...
    def test_get_energy_output(self):
        temp = 10
        test_data = (
            (399, 3_990),
            (300, 3_000),
        ...
        for variant, (neutrons_per_second, expected) in enumerate(test_data, start=1):
            with self.subTest(
                ...
                # pylint: disable=assignment-from-no-return
                actual_result = get_energy_output(temp, neutrons_per_second)
                failure_message = (
                    f"Expected {expected} but returned {actual_result} with T={temp}, "
                    f"neutrons={neutrons_per_second}"
                )
                self.assertEqual(actual_result, expected, failure_message)
