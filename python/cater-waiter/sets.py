...
try:
    from .sets_categories_data import ALCOHOLS, KETO, OMNIVORE, PALEO, SPECIAL_INGREDIENTS, VEGAN, VEGETARIAN
...
def categorize_dish(dish_name: str, dish_ingredients: List[str]) -> str:
    ...
    """
    # using varname module would be eaiser than manually mapping a string to the module name
    ...
    category_ingredients: Set[str]
    category_name: str

    for category_name, category_ingredients in data.items():
...
def singleton_ingredients(dishes: List[Set[str]], intersection: List[str]) -> Set[str]:
    ...
    """
    ingredients: List[Set[str]] = [dish - set(intersection) for dish in dishes]

    return set.union(*ingredients)
