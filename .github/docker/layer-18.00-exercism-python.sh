#!/bin/bash
#
# .github/docker/layer-18.00-exercism-python.sh
#
...
main() {
	...
	echo Running: mv .pyenv "${PYENV_ROOT}"
	time mv "${HOME}/.pyenv" "${PYENV_ROOT}" || track_errors
	printf "\n"
	...
	echo '. /etc/profile.d/python.sh' | tee -a "${HOME}/.bashrc" | tee -a "${HOME}/.profile" | tee -a /etc/skel/.bashrc
	printf "\n"
...
time main "${@}" |& tee "${HOME}"/layer-18.00-exercism-python.log
